...
stmt : assignStmt {k++;id=0;}| ifStmt {k++;id=0;}| whileStmt {k++;id=0;}| ifelseStmt {k++;id=0;}| ifelseifStmt {k++;id=0;}| forStmt {k++;id=0;};
assignStmt : ID '=' EXP SEMICOLON {if(checkexist($1)==-1)printf("\nError: Not declared\n");else{ struct Node * adder;strcpy(adder->vartype,vbltable[checkexist($1)]);strcpy(adder->varname,$1);if(end!=NULL){end->next=adder;end=end->next;}else{end=adder;typestorer=adder;} }};
EXP : EXP '+' TERM | EXP '-' TERM | TERM;
TERM : TERM '*' FACTOR | TERM '/' FACTOR | FACTOR;
FACTOR : ID {if(checkexist($1)==-1)printf("\nError: %s Not declared\n",$1);else{ struct Node * adder;strcpy(adder->vartype,vbltable[checkexist($1)]);strcpy(adder->varname,$1);if(end!=NULL){end->next=adder;end=end->next;}else{end=adder;typestorer=adder;} }}| INTEGER_CONSTANT;
...
bExp : EXP RELOP EXP {k++;id=0;}| EXP LOGOP EXP {printf("Types in line:\n");while(typestorer!=NULL){printf("%s - %s",typestorer->varname,typestorer->vartype);typestorer=typestorer->next;};};
